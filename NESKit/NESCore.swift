...
// --- Libretro Types for NES ---
typealias nes_retro_environment_t = @convention(c) (UInt32, UnsafeMutableRawPointer?) -> Bool
typealias nes_retro_video_refresh_t = @convention(c) (UnsafeRawPointer?, UInt32, UInt32, Int) -> Void
typealias nes_retro_audio_sample_t = @convention(c) (Int16, Int16) -> Void
typealias nes_retro_audio_sample_batch_t = @convention(c) (UnsafePointer<Int16>?, Int) -> Int
typealias nes_retro_input_poll_t = @convention(c) () -> Void
typealias nes_retro_input_state_t = @convention(c) (UInt32, UInt32, UInt32, UInt32) -> Int16

struct nes_retro_game_info {
...
struct nes_retro_system_av_info {
    struct Geometry {
    ...
    struct Timing {
        var fps: Double = 60.0
        var sample_rate: Double = 48000.0
    }
...
private let nesRendererInstance = NESRenderer()
private var nesFirstFrame = true

// --- C Callbacks ---

@_cdecl("nes_video_refresh")
func nes_video_refresh(data: UnsafeRawPointer?, width: UInt32, height: UInt32, pitch: Int) {
    ...
    if pitch > 0 && width > 0 && height > 0 {
        ...
        if nesFirstFrame {
            print("🎬 [NES Video] First frame: \(width)x\(height), pitch=\(pitch), BPP=\(bpp)")
            nesFirstFrame = false
            
            // Switch format if detected 32-bit (BPP ~4)
            if bpp >= 4 {
                nesRendererInstance.setPixelFormat(.bgra8Unorm)
            } else {
                nesRendererInstance.setPixelFormat(.b5g6r5Unorm)
            }
        }
        
        nesRendererInstance.updateTexture(width: Int(width), height: Int(height), pitch: pitch, data: data)
    }
}

private var nesTempAudioBuffer: [Int16] = [0, 0]

@_cdecl("nes_audio_sample")
func nes_audio_sample(left: Int16, right: Int16) {
    nesTempAudioBuffer[0] = left
    nesTempAudioBuffer[1] = right
    NESAudio.shared.writeAudio(data: nesTempAudioBuffer, frames: 1)
}

@_cdecl("nes_audio_sample_batch")
func nes_audio_sample_batch(data: UnsafePointer<Int16>?, frames: Int) -> Int {
    guard let data = data else { return 0 }
    NESAudio.shared.writeAudio(data: data, frames: frames)
    return frames
}

@_cdecl("nes_input_poll")
func nes_input_poll() {
    NESInput.shared.pollInput()
}

@_cdecl("nes_input_state")
func nes_input_state(port: UInt32, device: UInt32, index: UInt32, id: UInt32) -> Int16 {
    // Port 0 = Player 1
    if port == 0 {
        if device == 1 || device == 0 { // Joypad
            let mask = UInt16(1 << id)
            return (NESInput.shared.buttonMask & mask) != 0 ? 1 : 0
        }
    }
    return 0
}

@_cdecl("nes_environment")
func nes_environment(cmd: UInt32, data: UnsafeMutableRawPointer?) -> Bool {
    switch cmd {
    case 10: // RETRO_ENVIRONMENT_SET_PIXEL_FORMAT
        if let data = data {
            let format = data.bindMemory(to: UInt32.self, capacity: 1).pointee
            // 2 = RGB565 (Standard for NES cores)
            if format == 2 {
                // print(" [NES] Core set RGB565")
                return true
            }
        }
        return false
    case 31: // RETRO_ENVIRONMENT_GET_SAVE_DIRECTORY
        if let data = data {
            if let docDir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first {
                let saveDir = docDir.appendingPathComponent("saves/nes")
                ...
                print("📂 [NESCore] Save Directory set to: \(pathStr)")
                return true
            }
        }
        return false
...
public class NESCore: ObservableObject {
    @Published public var isRunning = false
    private var coreHandle: UnsafeMutableRawPointer?
    private var displayLink: CADisplayLink?
    
    public var renderer: NESRenderer {
        return nesRendererInstance
    }
    
    // Core Functions
    private var retro_init: (@convention(c) () -> Void)?
    private var retro_deinit: (@convention(c) () -> Void)?
    private var retro_set_environment: (@convention(c) (nes_retro_environment_t) -> Void)?
    private var retro_set_video_refresh: (@convention(c) (nes_retro_video_refresh_t) -> Void)?
    private var retro_set_audio_sample: (@convention(c) (nes_retro_audio_sample_t) -> Void)?
    private var retro_set_audio_sample_batch: (@convention(c) (nes_retro_audio_sample_batch_t) -> Void)?
    private var retro_set_input_poll: (@convention(c) (nes_retro_input_poll_t) -> Void)?
    private var retro_set_input_state: (@convention(c) (nes_retro_input_state_t) -> Void)?
    ...
    private func loadCore() {
        var corePath: String?
        let libName = "fceumm_libretro_ios"
        
        // 1. Check Frameworks Directory (Preferred for Embedded Dylibs)
        if let frameworksURL = Bundle.main.privateFrameworksURL {
            // Case A: Inside a .framework wrapper (Standard)
            let frameworkPath = frameworksURL.appendingPathComponent("FCEUmm.framework/FCEUmm").path
            if FileManager.default.fileExists(atPath: frameworkPath) {
                print(" [NESCore] Found in Frameworks: \(frameworkPath)")
                corePath = frameworkPath
            }
            // Case B: Fallback
            else {
                let flatPath = frameworksURL.appendingPathComponent("fceumm_libretro_ios.dylib").path
        ...
        guard let validPath = corePath else {
            print(" [NESCore] Could not find FCEUmm library.")
            return
        }
        
        print("📂 [NESCore] Loading core from: \(validPath)")
        
        coreHandle = dlopen(validPath, RTLD_NOW)
        guard coreHandle != nil else {
            let error = String(cString: dlerror())
            print(" [NESCore] dlopen failed: \(error)")
            return
        }
        ...
        // Setup Callbacks
        retro_set_environment?(nes_environment)
        retro_set_video_refresh?(nes_video_refresh)
        retro_set_audio_sample?(nes_audio_sample)
        retro_set_audio_sample_batch?(nes_audio_sample_batch)
        retro_set_input_poll?(nes_input_poll)
        retro_set_input_state?(nes_input_state)
        
        retro_init?()
        print(" [NESCore] Initialized")
    }
    ...
    func loadGame(url: URL) -> Bool {
        ...
        return path.withUnsafeBufferPointer { pathPtr in
            ...
            guard let success = withUnsafePointer(to: &info, { retro_load_game?($0) }), success else {
                print(" [NES] Failed to load game path")
                return false
            }
            
            var avInfo = nes_retro_system_av_info()
            ...
            NESAudio.shared.start(rate: avInfo.timing.sample_rate)
    ...
    public func pause() {
        if isRunning {
            ...
            NESAudio.shared.stop()
             print("sh [NESCore] Paused")
        }
    }
    ...
    public func resume() {
        if !isRunning && displayLink != nil {
            ...
            NESAudio.shared.start(rate: currentSampleRate) // Use valid rate
             print("▶️ [NESCore] Resumed at \(currentSampleRate)Hz")
        } else if !isRunning {
            startLoop(fps: currentFPS)
            NESAudio.shared.start(rate: currentSampleRate)
        }
    }
    ...
    func loadSaveRAM() {
        ...
        if let ptr = getData(0) {
            if let data = try? Data(contentsOf: path), data.count <= size {
                ...
                print(" [NESCore] Save RAM loaded: \(path.lastPathComponent)")
            }
        }
    }
    ...
    func saveSaveRAM() {
        ...
        if let ptr = getData(0) {
            ...
            do {
                ...
                print(" [NESCore] Save RAM saved: \(path.lastPathComponent)")
            } catch {
                print(" [NESCore] Failed to write Save RAM: \(error)")
            }
        }
    }
}
