...
public struct PicoDriveRenderView: UIViewRepresentable {
    public let renderer: PicoDriveRenderer
...
public class PicoDriveRenderer: NSObject, MTKViewDelegate {
    ...
    public func setPixelFormat(_ format: MTLPixelFormat) {
        if currentPixelFormat != format {
            print(" [PicoDriveRenderer] Switching Pixel Format to: \(format == .bgra8Unorm ? "BGRA8888 (32-bit)" : "RGB565 (16-bit)")")
    ...
    func buildPipeline() {
        ...
        guard let library = device.makeDefaultLibrary(),
              let vertexFunc = library.makeFunction(name: "pico_vertex"),
              let fragmentFunc = library.makeFunction(name: "pico_fragment") else { return }
        ...
        do {
            ...
            print(" [PicoDriveRenderer] Pipeline Error: \(error)")
        }
    }
    ...
    func updateTexture(width: Int, height: Int, pitch: Int, data: UnsafeRawPointer) {
        ...
        if texture == nil || texture?.width != width || texture?.height != height {
            ...
            print("🎬 [PicoDriveRenderer] Created texture: \(width)x\(height), fmt=\(currentPixelFormat == .bgra8Unorm ? "32-bit" : "16-bit")")
        }
