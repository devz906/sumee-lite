...
public class Air {
    ...
    public static func clearListeners() {
        Air.shared.connectionCallbacks.removeAll()
    }
    
    public static func connection(_ callback: @escaping (Bool) -> ()) {
        Air.shared.connectionCallbacks.append(callback)
        // Immediately invoke with current state so the view knows if it's already connected
        callback(Air.shared.connected)
        // Also force a check just in case state is stale
        Air.shared.check()
    }
