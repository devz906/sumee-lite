...
struct ConsoleSelectionOverlay: View {
    @Binding var isPresented: Bool
    @Binding var selectedConsole: ROMItem.Console?
            ...
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    VStack(spacing: 6) {
                        ...
                        Text("Choose a console to customize")
                            .font(.subheadline)
                    ...
                    ScrollViewReader { proxy in
                        ScrollView {
                            LazyVStack(spacing: 12) {
                                ForEach(Array(themableConsoles.enumerated()), id: \.element) { index, console in
                                    let hasCustom = settings.getCustomConsoleIconPath(for: console, ignoreActiveTheme: true) != nil
                                    let isFocused = (selectionIndex == index)
                                    
                                    Button(action: {
                                        handleSelection(console: console)
                                    }) {
                                        HStack {
                                            // Icon
                                            ZStack {
                                            ...
                                            Text(console.rawValue)
                                                ...
                                                .foregroundColor(isFocused ? themeBlue : textMain)
    ...
    private func handleSelection(console: ROMItem.Console) {
        selectedConsole = console
        showIconPicker = true
        AudioManager.shared.playSelectSound()
        // Kept open to allow multiple edits
    }
}
